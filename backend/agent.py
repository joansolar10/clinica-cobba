...
def router_node(state: AgentState) -> AgentState:
    ...
    prompt = f"""Eres un clasificador de intenciones para una clínica odontológica peruana.
...
def scheduler_node(state: AgentState) -> AgentState:
    ...
    if step == "asking_specialty":
        prompt = f"""Extrae la especialidad odontológica del mensaje.
Las especialidades disponibles son: Odontología General, Ortodoncia, Endodoncia, Periodoncia, Implantología, Odontopediatría, Cirugía Oral.
Si no se menciona ninguna o no está clara, responde exactamente: Odontología General
            ...
            if doc_req and not date_req:
                ...
                if doc_slots:
                    ...
                    response = (
                        ...
                        f"¿Te gustaría ver opciones con otro odontólogo?\n\n{slots_text}"
                    )
                history.append({"role": "assistant", "content": response})
                return {**state, "extracted": extracted, "step": "choosing_slot",
                        "response": response, "conversation_history": history}
...
Sé breve y directo. Responde en español."""
    ...
    if step == "ready_to_schedule":
        ...
        prompt = f"""El usuario responde a una confirmación de cita odontológica.
...
Las especialidades disponibles son: Odontología General, Ortodoncia, Endodoncia, Periodoncia, Implantología, Odontopediatría y Cirugía Oral.
